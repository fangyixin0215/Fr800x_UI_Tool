...
//		this.sortPal = ['hue-','lum-','sat-'];
	...
	RgbQuant.prototype.reduce = function reduce(img, retType, dithKern, dithSerp) {
		...
		// reduce w/dither
		if (dithKern) {
			if (dithKern.indexOf("Ordered") == 0)
				var out32 = this.ditherOrdered(img, dithKern, dithSerp);
			else
				var out32 = this.ditherDiffusion(img, dithKern, dithSerp);
		}
		else {
	...
	RgbQuant.prototype.ditherDiffusion = function(img, kernel, serpentine) {
...
//			buf8 = data.buf8,
	...
	RgbQuant.prototype.ditherOrdered = function(img, kernel, serpentine) {
		function clamp(val) {
		...
		var rows = +kernel.substr(-1),
			cols = +kernel.substr(-1),
			div = rows * cols + 1,
			mfactor = 256 / 4;
...
//			buf8 = data.buf8,
		...
		for (var y = 0; y < height; y++) {
			...
			for (var x = 0; x < width; x++) {
				...
				var th = ds[x % rows][y % cols];

				var r2 = Math.round(clamp(r1 + th / div * mfactor)),			// * mfactor
					g2 = Math.round(clamp(g1 + th / div * mfactor)),
					b2 = Math.round(clamp(b1 + th / div * mfactor)),
					i32x = this.nearestColor(
						(
							(255 << 24) |	// alpha
							(b2  << 16) |	// blue
							(g2  <<  8) |	// green
